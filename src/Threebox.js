...
Threebox.prototype = {
  ...
  init: function (map, glContext, options) {
    ...
    this.map.on("load", function () {
      ...
      this.outFeature = function (f) {
        if (
          this.overedFeature &&
          typeof this.overedFeature != "undefined" &&
          this.overedFeature.id != f
        ) {
          map.setFeatureState(
            {
      ...
      this.onClick = function (e) {
        ...
        if (intersectionExists) {
          ...
          if (features.length > 0) {
            if (
              features[0].layer.type == "fill-extrusion" &&
              typeof features[0].id != "undefined"
            ) {
      ...
      this.onMouseMove = function (e) {
        ...
        // [jeffmacinnes] assign the mouse position (relative to map) to map.mousePos so that it is accessible;
        this.mousePos = current;
        ...
        if (e.originalEvent.altKey && this.draggedObject) {
          ...
          let rotation = {
            ...
            z: Math.round(
              rotationDiff[2] +
                (((~~((current.x - start.x) / this.tb.rotationStep) % 360) * this.tb.rotationStep) %
                  360)
            ),
          };
      ...
      this.onMouseDown = function (e) {
        ...
        if (
          !(
            (e.originalEvent.shiftKey || e.originalEvent.altKey || e.originalEvent.ctrlKey) &&
      ...
      this.onMouseUp = function (e) {
        ...
        if (this.draggedObject) {
          this.draggedObject.dispatchEvent({
            type: "ObjectDragged",
            detail: { draggedObject: this.draggedObject, draggedAction: draggedAction },
          });
    ...
    // [jeffmacinnes] map resize to re-sync the camera to the viewport
    // https://github.com/jscastro76/threebox/issues/383
    this.map.on("resize", () => {
  ...
  setSunlight: function (newDate = new Date(), coords) {
    if (!this.lights.dirLight || !this.options.realSunlight) {
      console.warn(
        "To use setSunlight it's required to set realSunlight : true in Threebox initial options."
      );
      return;
    }
      ...
      this.map.setLight(
        {
          ...
          position: [
            3,
            180 + (this.sunPosition.azimuth * 180) / Math.PI,
            90 - (this.sunPosition.altitude * 180) / Math.PI,
          ],
          intensity: Math.cos(this.sunPosition.altitude), //0.4,
          color: `hsl(40, ${50 * Math.cos(this.sunPosition.altitude)}%, ${Math.max(
            20,
            20 + 96 * Math.sin(this.sunPosition.altitude)
          )}%)`,
        },
        { duration: 0 }
      );
  ...
  updateSunGround: function (sunPos) {
    if (this.terrainLayerName != "") {
      ...
      this.map.setPaintProperty(
        this.terrainLayerName,
        "raster-opacity",
        Math.max(Math.min(1, sunPos.altitude * 4), 0.25)
      );
    }
  },
